...
def optimization_plot(ex : Experiment, savefig = True):
    ...
    if len(ex.agent()._global_costs) == 0:
        return
    ...
    # global cost plot
    gca : plt.Axes = ax4[0]
    gca.plot([ i+1 for i in range(len(ex.agent()._global_costs))], ex.agent()._global_costs, linewidth=2)
    gca.set_ylabel('$J$')
    
    # gradient plot
    ggn : plt.Axes = ax4[1]
    xticks = np.cumsum(ex.agent()._steady_state_iters)
    ggn.step(xticks, ex.agent()._global_gradient_norms, linewidth=2, color='blue')
    ggn.set_ylabel('$\\| \\nabla_\\tau J \\|_\\infty$')
    ggn.set_yscale('log')
    ...
    ytau = np.concatenate(([ex.agent()._tau_vals[0][:]], np.array(ex.agent()._tau_vals)))
    
    for i in range(ytau.shape[1]):
        tva.step(xtau, ytau[:,i], linewidth=2, color=plotAttr.target_colors[-i])
    tva.set_ylabel('$ \\tau$')
    ...
    ex.agent().addSteadyStateLines(ax4[0], alpha=0.1, color='black')
    ex.agent().addSteadyStateLines(ax4[1], alpha=0.1, color='black')
    ex.agent().addSteadyStateLines(ax4[2], alpha=0.1, color='black')
...
if __name__ == '__main__':
    plotResults(load_optimized_cycle(load_high_level_solution(load_experiment())))
    tsp_vs_init_vs_opti()
    mse_inti_vs_opti() 
    # rrt_plot()
